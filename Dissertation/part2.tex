...
\subsection{Embeddings} \label{subsect2_1_2}
...
\label{eq:equation2_8}
  \begin{multlined}
	\frac{\partial(\textrm{CE})}{\partial{{x}_{k}}} = -\frac{\partial(-\sum_{i}{y_{i}\log{\hat{y_{i}}}})}{\partial{{x}_{k}}} = -\sum_{i}{y_{i}\frac{\partial \hat{y_{i}}}{\partial{{x}_{k}}}}\frac{1}{\hat{y_{i}}} = \\
	= -y_k(1-\hat{y_k}) - \sum_{i \neq k}{y_i\frac{1}{\hat{y_i}}}{(-\hat{y_k}\hat{y_i})} 
	= -y_k(1-\hat{y_k}) + \sum_{i \neq k}{y_i\hat{y_k}} = \\
	= \hat{y_k}\sum_{i}{y_i} - y_k 
  \end{multlined}
\end{equation}

Requiring $y_{k}$ to be non-zero, imposes that the auxiliary condition, $k=i$ and $y_{k}=1$, hence it follows immediately,
\begin{equation}
\label{eq:equation2_9}
\frac{\partial(\textrm{CE})}{\partial{x_{k}}} = \begin{cases}
(\hat{y}_{k} - 1)&\text{ for }k=i \\
\hat{y}_{k}&\text{ for }k\neq i
\end{cases}
\end{equation}
Which is equivalent to 
\begin{equation}
\frac{\partial(\textrm{CE})}{\partial{X}} = \bf{\hat{y}} - \bf{y}
\end{equation}
...
\frac{\partial{\bf z}_{i}}{\partial{{\bf x}^{(i)}}} = {\bf \textrm{W}^{\top}_{i}}
\end{equation}
