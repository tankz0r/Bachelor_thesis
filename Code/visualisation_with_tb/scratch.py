...
def top_k_metric(y_true, y_pred, **kwargs):
    kk= kwargs.get('k', 3)
    return kmetrics.top_k_categorical_accuracy(y_true, y_pred, k=kk)
    
def scratch(y_true, y_pred):
    import tensorflow as tf
    return tf.reduce_sum(y_true[:,0], name='tf_red_sum')
...
if __name__ == "__main__":
    ...
    # Logging
    #log_metrics = ['categorical_accuracy', 'categorical_crossentropy', top_k_metric, scratch]
...
if use_google_word2vec and __name__ == "__main__":
    ...
    if not os.path.exists(word2vec_model_path):
        ...
        model = KeyedVectors.load_word2vec_format(google_word2vec, limit=max_vocab_size, binary=True)
...
if __name__ == "__main__":
    ...
    if initial_epoch < epochs:
        ...
        history = model.fit_generator(train_generator, batches_per_epoch, epochs, callbacks=_callbacks, initial_epoch=initial_epoch)
