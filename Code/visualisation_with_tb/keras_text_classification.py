...
def briers(y_true, y_pred):
    ...
    if y_true is not None:
        ...
        inds = tf.stack([tf.to_int64(tf.range(tf.shape(y_pred)[0])), classes_true])
...
if True and __name__ == "__main__":
    ...
    if not os.path.exists(word2vec_model_path):
        model = KeyedVectors.load_word2vec_format(google_word2vec, limit=top_words, binary=True)
...
if True and __name__ == "__main__":
    ...
    log_metrics = ['categorical_accuracy', 'categorical_crossentropy', batch_pairwise_metrics]
